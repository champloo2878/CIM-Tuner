...
dbx = np.arange(len(networks)*len(macros) + 1)  # 7+7+1=15个位置
width = 0.3
group_width = width * 3 
dbnetwork = networks + networks + ['mean']  # 添加mean标签
...
for idx, metric in enumerate(metrics):
    ...
    mean_pos = len(networks) * len(macros)
    ax.bar(mean_pos - width/2, mean_best_fix[idx], width, color = "#696969")
    ax.bar(mean_pos + width/2, mean_best[idx], width, hatch = '//', color = '#b7b7b7')
